...
#get information
    ...
    def assign_empty(self):
        ...
        self.moneydict = {2016 : np.nan, 2017 : np.nan, 2018 : np.nan}  #매출액인듯
        self.eieikdict = {2016 : np.nan, 2017 : np.nan, 2018 : np.nan}
        self.sokdict = {2016 : np.nan, 2017 : np.nan, 2018 : np.nan}
        self.pskdict = {2016 : np.nan, 2017 : np.nan, 2018 : np.nan}
        self.sooneieikdict = {2016: np.nan, 2017: np.nan, 2018: np.nan}
        jujulist = []
        groupdict = {}
        for i in range(3):
    ...
    def get_overview_info(self):
        ...
        if moneydict == "N/A":
            moneydict = {}
            moneydict[2016], moneydict[2017], moneydict[2018] = "N/A", "N/A", "N/A"
        ...
        if eieikdict == "N/A":
            eieikdict = {}
            eieikdict[2016], eieikdict[2017], eieikdict[2018] = "N/A", "N/A", "N/A"
        else:
            eieikdictyears = sorted(list(eieikdict.keys()))
            lastyear = eieikdictyears[-1]
            if lastyear < 2016:
                eieikdict[2016] = str(lastyear) + "년   " + str(eieikdict[lastyear])
                eieikdict[2016], eieikdict[2017], eieikdict[2018] = "N/A", "N/A", "N/A"
            else:
                for i in [2016, 2017, 2018]:
        ...
        if sokdict == "N/A":
            sokdict = {}
            sokdict[2016], sokdict[2017], sokdict[2018] = "N/A", "N/A", "N/A"
        else:
            sodictyears = sorted(list(sokdict.keys()))
            lastyear = sodictyears[-1]
            if lastyear < 2016:
                sokdict[2016] = str(lastyear) + "년   " + str(sokdict[lastyear])
                sokdict[2016], sokdict[2017], sokdict[2018] = "N/A", "N/A", "N/A"
            else:
                for i in [2016, 2017, 2018]:
        ...
        if pskdict == "N/A":
            pskdict = {}
            pskdict[2016], pskdict[2017], pskdict[2018] = "N/A", "N/A", "N/A"
        else:
            psdictyears = sorted(list(pskdict.keys()))
            lastyear = psdictyears[-1]
            if lastyear < 2016:
                pskdict[2016] = str(lastyear) + "년   " + str(pskdict[lastyear])
                pskdict[2016], pskdict[2017], pskdict[2018] = "N/A", "N/A", "N/A"
            else:
                for i in [2016, 2017, 2018]:
        ...
        if sooneieikdict == "N/A":
            sooneieikdict = {}
            sooneieikdict[2016], sooneieikdict[2017], sooneieikdict[2018] = "N/A", "N/A", "N/A"
        else:
            sooneieikyears = sorted(list(sooneieikdict.keys()))
            lastyear = sooneieikyears[-1]
            if lastyear < 2016:
                sooneieikdict[2016] = str(lastyear) + "년   " + str(sooneieikdict[lastyear])
                sooneieikdict[2016], sooneieikdict[2017], sooneieikdict[2018] = "N/A", "N/A", "N/A"
            else:
                for i in [2016, 2017, 2018]:
...
class DataFrameOperator:
    ...
    def prepare_tax_excel(self):
        ...
        path = self.dir + r'\\asset\\static\\Korea GB Account CRM Extraction File_20200212.xlsx'
        ...
        if taxnumber == "N/A":
            ...
            try:
                ...
                for i in range(len(taxdict)):
                    print(f"동일한 사업자번호를 가진 BP#  {taxdict[i]['ORG ID (BP#)']} | {taxdict[i]['ORG Name (ENG)']} | {taxdict[i]['ORG Name (KOR)']} 가 발견되었습니다.")
    ...
    def prepare_kscode_excel(self):
        ...
        path = self.dir + r'\\asset\\static\\10차산업분류코드 Mapping File_20191218.xlsx'
    ...
    def match_kscode(self, kscode):
        if kscode == "N/A":
            ...
            try:
                mccode = self.kscodefile[self.kscodefile["10차 KS Code"] == f'{kscode}']["Master Code"].get_values()[0]
    ...
    def make_infomation(self, information):
        basic_form = collections.OrderedDict({
           ...
           # 'Local Sales Turnover\n2015 (K won)': information.moneydict[2015],  #고친부분
            ...
            'Local Sales Turnover\n2018 (K won)': information.moneydict[2018],
            'EUR Sales Turnover 2016': np.nan,
            #'영업이익\n2015 (K won)': information.eieikdict[2015],  #고친부분
            #'영업이익\n2016 (K won)': information.eieikdict[2016],
            #'영업이익\n2017 (K won)': information.eieikdict[2017],
            #'EUR Sales Turnover 2016': np.nan,
            #'영업이익율 \n2015 (K won)': information.sokdict[2015],  #고친부분
            #'영업이익율\n2016 (K won)': information.sokdict[2016],
            #'영업이익율\n2017 (K won)': information.sokdict[2017],
            #'순이익률 \n2015 (K won)': information.pskdict[2015],  #고친부분
            #'순이익률\n2016 (K won)': information.pskdict[2016],
            #'순이익율\n2017 (K won)': information.pskdict[2017],
            ...
            'Contact Email Address\n(40 characters)': 'noemail@noemail.com',
            ...
            'Contact Person \nMobile Phone Number(no Country code indicator necessary)': '000-000-0000',
    ...
    def make_social_infomation(self, information):
        if len(information.taxlist) > 0:
            ...
            RBC = "Net New"
        basic_form = collections.OrderedDict({
            'Profiling Intern Name' : '정가을',
            'Profile Date': self.today,
            'BP# Status (Y/N)': creation,
            'Week' : '',
            'AE Name (2019 CRM)' : '',
            'DDE Name' : '',
            'BP# (ORG ID)' : str(information.taxlist),
            'Account Name (KOR)': information.company_name_kor,
            'Account Name (ENG)': information.company_name_eng,
            '담당자 이름' : '',
            '직함' : '',
            '부서' : '',
            '대표번호' : information.phonenumber,  
            '내선번호' : '',
            '핸드폰번호' : '',
            '이메일' : '',
            'DDE Call Log' : '',
            'Profile Status(Drop-down)' : '',
            'ERP(벤더사)' : '',
            'Concur Touch' : '',
            'Campaign Name' : 'Social Listening',
            'Year' : '2019',
            'Social Listening Lead Description' : '',
            '기사 URL link' : '',
            'Keyword1' : '',
            'Keyword2' : '',
            'Keyword3' : '',
            ...
            'Local Sales Turnover\n2017 (K won)': information.moneydict[2017],
            '직원수' : information.number_of_employee,
            'Master Code Text (CRM)' : '#N/A',
            'KS Code 10차 (KISLINE)' : information.kscode,
            'KS Code Text (KISLINE)' : information.kscode_kor,
            'MC Mapping (KISLINE)' : information.mccode_discription, 
            'AE Mapping (KISLINE)' : str(information.AElast) + ', ' + str(information.AEfirst),
            '2019 AE MATCH (CRM-KISLINE)' : '',
            'CRM MC_KIS MATCH' : '',
            '영남' : information.youngnam,
            '주소' : information.address_kor,
            '홈페이지' : information.homepage,
            'Buying Classification (CRM)' : RBC,
            'Account Classification 2019 (CRM)' : information.IAC
    ...
    def make_infomation_I_love_UGB(self, information):
        ...
        basic_form = collections.OrderedDict({
            ...
            'Industry Description ENG\n(표준산업분류)\n(KISLINE data)': information.kscode_eng,
            'Local Sales Turnover\n2016 (K won)': information.moneydict[2016],
            'Local Sales Turnover\n2016 (K won)': information.moneydict[2017],
            'Local Sales Turnover\n2017 (K won)': information.moneydict[2018],
            'EUR Sales Turnover 2016': np.nan,
            'Local Employee Size': information.number_of_employee,
            'Local Employee Size Year': information.year_of_employee,
            'Global Employee Size': np.nan,
            ...
            'Turnover2018 (K won)': information.moneydict[2018],
            'OperationProfiturnover2018 (K won)': information.eieikdict[2018],
            'NetProfit2018 (K won)': information.sooneieikdict[2018],
...
if __name__ == "__main__":
    ...
    #df = pd.read_excel(r'C:\Users\i531745\Documents\crawling test\m&a_list.xlsx')
    #companylist = ['(주)엘앤씨바이오', '(주)하우저', '(주)티에스트릴리온', '(주)크린앤사이언스',
    #    '(주)네패스라웨', '(주)아이에이', '(주)기가레인', '(주)셀레믹스', '(주)지엔티파마',
    #    '스타코(주)', '(주)뉴아인', '(주)다원메닥스', '(주)한컴라이프케어']
    ...
    if search_term == str(9999):
        ...
        while mode != 0:
            try:
                ...
                print(e)
                traceback.print_exception(*sys.exc_info())
                print('Error is occured. Please report to jiyeon.lee@sap.com')
                break
